...
{ 
	...
	_color = switch (_team) do {
		case "RED": {f_groupColorR_Nametags};
		case "GREEN": {f_groupColorG_Nametags};
		case "BLUE": {f_groupColorB_Nametags};
		case "YELLOW": {f_groupColorY_Nametags};
		case "MAIN": {f_groupColor_Nametags};
		default  {f_groupColor_Nametags};
	};
}; // Units of same group
...
if (_showgroup && group _u != group player) then {_str = format ["%1 | ",toUpper(groupID (group _u))] + _str};
